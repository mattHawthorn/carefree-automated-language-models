...
class Processor:
    ...
    def __init__(self,configFile=None,stopwordsFile=None,**kwargs):
        ...
        if 'ngrams' in kwargs:
            ngramConfig = cl.clean_args(kwargs['ngrams'],ngramThesaurus)
        ...
        for i, op in enumerate(sequence):
            ...
            if params:
                params = cl.clean_args(op['args'],argThesauri[operation])
    ...
    def __case(self,strings,args):
        f = args[0] # the case function
        return [f(s) for s in strings]

    def __caseMatched(self,strings,args):
        f = args[0] # the case function
    ...
    def __removeStopwords(self,strings,args):
        return [s for s in strings if s not in self.stopwords]

    def __stem(self,strings,args):
        return [self.stem(s) for s in strings]

    def __tokenize(self,strings,args):
        return chain.from_iterable([self.tokenize(s) for s in strings])
